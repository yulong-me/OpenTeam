...
const frontendSocketServerSource = readFileSync(resolve(root, 'desktop/frontend-socket-server.cjs'), 'utf8');
...
assert.match(mainSource, /resolveAppPath\('desktop', 'frontend-socket-server\.cjs'\)/, 'desktop launcher must start the frontend socket server without shelling through pnpm');
assert.match(mainSource, /resolveAppPath\('frontend', '\.next', 'standalone'\)/, 'packaged desktop launcher must serve the Next standalone app over a socket');
...
assert.match(mainSource, /BACKEND_SOCKET_PATH/, 'desktop launcher must run backend on an internal socket instead of a TCP port');
assert.match(mainSource, /FRONTEND_SOCKET_PATH/, 'desktop launcher must run frontend on an internal socket instead of a TCP port');
...
assert.match(frontendSocketServerSource, /FRONTEND_SOCKET_PATH/, 'desktop frontend server must listen on a Unix socket');
assert.match(frontendSocketServerSource, /next\(/, 'desktop frontend server must reuse the Next request handler');
assert.match(backendServerSource, /BACKEND_SOCKET_PATH/, 'backend server must support a Unix socket for desktop');
